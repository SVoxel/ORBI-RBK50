...
IoT_Error_t aws_iot_add_Objects(char *pJsonDocument, size_t maxSizeOfJsonDocument, uint8_t count, ...) {
...
IoT_Error_t aws_iot_add_Object(char *pJsonDocument, size_t maxSizeOfJsonDocument, jsonStruct_t *pObject) {
	IoT_Error_t ret_val = SUCCESS;
	int32_t snPrintfReturn = 0;
	...
	snPrintfReturn = snprintf(pJsonDocument, maxSizeOfJsonDocument, "\"%s\":{", pObject->pKey);
...
IoT_Error_t aws_iot_add_Object_buf(char *pJsonDocument, size_t maxSizeOfJsonDocument, char *json) {
	IoT_Error_t ret_val = SUCCESS;
	int32_t snPrintfReturn = 0;
	...
	snPrintfReturn = snprintf(pJsonDocument, maxSizeOfJsonDocument, "\"%s\"", json);

	ret_val = checkReturnValueOfSnPrintf(snPrintfReturn, maxSizeOfJsonDocument);
	ret_val = convertDataToString(pJsonDocument + strlen(pJsonDocument), maxSizeOfJsonDocument, SHADOW_JSON_STRING, json);

	return ret_val;
}

IoT_Error_t aws_iot_finalize_Object(char *pJsonDocument, size_t maxSizeOfJsonDocument) {
	IoT_Error_t ret_val = SUCCESS;
	int32_t snPrintfReturn = 0;
	...
	snPrintfReturn = snprintf(pJsonDocument, maxSizeOfJsonDocument, "}");
