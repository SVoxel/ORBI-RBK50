...
#ifdef USE_ATH_HEADER
//#define SWINTERFACE	DGC_IF_WAN_PHYNAME
...
int main( int ArgCn, const char *ArgVc[] ) {
    ...
    strncpy(wan_ifname, config_get("dgc_netif_wan_if"), sizeof(wan_ifname));
    strncpy(lan_ifname, config_get("dgc_netif_lan_if"), sizeof(lan_ifname));
    strncpy(ppp_ifname, config_get("dgc_netif_ppp_if"), sizeof(ppp_ifname));
    strncpy(wan_phyname, config_get("dgc_netif_wan_phyif"), sizeof(wan_phyname));
...
//	    }
	...
	if(commonConfig.mode&0x01){
		interfaceSetPacketFlag(wan_phyname, 0);
		igmpSnoopCleanUp();
	}
#endif
