...
function check_schedule_onoff()
{
	...
	if(led_schedule_num_a > 1)
	{
		for(i=0; i<led_schedule_num_a; i++)
		{
			cf.ruleSelect_5g[i].disabled = disable_flag;				
		}
	}
	else if(led_schedule_num_a > 0)
...
function check_led_schedule(btn_flag)
{
	var cf = document.forms["led"];

	//if(chan_type == "bgn")
	//{
		if(btn_flag == "add")
		{
			if(led_schedule_num >= 20)
			{
				alert("$led_max_rules");
				return false;
			}
			else
				location.href="led_sechdule_add.html";//fix bug 28245 not a por-up page
		...
		else if(btn_flag == "edit")
		{
			if(led_schedule_num > 0)
                        {
                                var i = 0;
				cf.led_schedule_edit_num.value = "";
				...
				if(led_schedule_num > 1)
				{
                                	for(i=0; i<led_schedule_num; i++)
                                	{
                                        	if(cf.ruleSelect_2g[i].checked)
                                                	cf.led_schedule_edit_num.value = parseInt(cf.ruleSelect_2g[i].value, 10) + 1;
                                	}
				}
				else
				{
					if(cf.ruleSelect_2g.checked)
						cf.led_schedule_edit_num.value = parseInt(cf.ruleSelect_2g.value, 10) + 1;
				}	

                                if(cf.led_schedule_edit_num.value == "")
                                {
					alert("$port_edit");
                                        return false;
                                }
                                ...
                                {
                                        cf.led_schedule_type.value = "edit_led";
                                        cf.submit_flag.value = "led_schedule_select_to_edit";
										//cf.action="apply.cgi?/led_controls.htm timestamp="+ts; 
										cf.action = "/apply.cgi?/led_schedule_edit.htm timestamp="+ts;
										cf.submit();
		...
		else if(btn_flag == "delete")
                {
                        ...
                        {
                                var i = 0;
				cf.led_schedule_delete_num.value = "";
				...
				if(led_schedule_num > 1)
				{
                                	...
                                	{
                                        	if(cf.ruleSelect_2g[i].checked)
                                                	cf.led_schedule_delete_num.value = parseInt(cf.ruleSelect_2g[i].value, 10) + 1;
                                	}
				}
				else
				{
					if(cf.ruleSelect_2g.checked)
						cf.led_schedule_delete_num.value = parseInt(cf.ruleSelect_2g.value, 10) + 1;
				}

                                if(cf.led_schedule_delete_num.value == "")
                                {
					alert("$port_del");
                                        return false;
                                }
                                ...
                                {
                                        cf.submit_flag.value = "led_schedule_delete";
                                        cf.action = "/apply.cgi?/led_controls.htm timestamp="+ts;
										cf.submit();
                                }
                        }
...
function check_schedule_legality(modify_flag, item_value, start, end, days)
{
	var array_name, array_num;
	array_name = "led_schedule_array";
	array_num = led_schedule_num;
	var array_value = "",array_valuest="", array_valueed = "",starts ="",ends="";
	...
	if(array_num > 0)
	{
		...
		{
			if ("Daily" == daysarray[j])
			{
				for(i=1; i<=array_num; i++)
				{	
					if((modify_flag=="edit" && i!=edit_num) || modify_flag == "add")
					{
						array_value = eval(array_name + i);
						array_valuest = parseInt(array_value.split("-")[0]);
						array_valueed = parseInt(array_value.split("-")[1]);
						starts = parseInt(start);
						ends = parseInt(end);
						if(array_valueed == 0)
							array_valueed = 48;//to easy check invalid
						if(ends == 0)
							ends = 48;
						if(starts >= ends)
						{
							...
							if(Math.max(array_valuest,starts) < Math.min(array_valueed,ends))
							{
								alert("$led_confilct");
								return false;
							}
			...
			{//2-8-0,3,4
				...
				{	
					if((modify_flag=="edit" && i!=edit_num) || modify_flag == "add")
					{
						...
						if ("Daily" == array_value.split("-")[2] || array_value.split("-")[2].indexOf(daysarray[j]) >= 0)
						{
							array_valuest = parseInt(array_value.split("-")[0]);
							array_valueed = parseInt(array_value.split("-")[1]);
							starts = parseInt(start);
...
function led_schedule_add(modify_flag)
{
	var cf = document.forms["led_addfrom"];
	var schedule_all = "", schedule_days = "";
	var i, j=0, starts=0, end=0;
	...
	starts = parseInt(cf.start_time.value);
	end = parseInt(cf.end_time.value);
	if(end == 0)
		end=48;
	...
	if(modify_flag == "add")
		schedule_all = cf.start_time.value + "-" + cf.end_time.value + "-" + schedule_days+ "-" + cf.lightbright.value+"-Default";
	else
		schedule_all = cf.start_time.value + "-" + cf.end_time.value + "-" + schedule_days+ "-" + cf.lightbright.value+"-"+cf.schrule.value;
	//schedule_all = cf.start_time.value + "-" + cf.end_time.value + "-" + schedule_days;
	schedule_all = remove_space(schedule_all);

	if(check_schedule_legality(modify_flag, schedule_all, cf.start_time.value, cf.end_time.value,schedule_days) == false)
		return false;
