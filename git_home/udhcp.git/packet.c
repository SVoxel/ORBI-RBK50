...
#ifdef UDHCP_USE_MUSL
	packet.udp.uh_ulen = htons(p_len);
	packet.ip.tot_len = packet.udp.uh_ulen;
	p_len += sizeof(packet.ip);
	packet.udp.uh_sum = checksum(&packet, p_len);
#else
...
#else
#ifdef UDHCP_USE_MUSL
	packet.udp.uh_ulen = htons(sizeof(packet.udp) + sizeof(struct dhcpMessage)); /* cheat on the psuedo-header */
	packet.ip.tot_len = packet.udp.len;
...
int copy_file(char *from_name, char *to_name)
{
	...
	from_fp = fopen(from_name, "r");
	to_fp = fopen(to_name, "w");
	if(!from_fp || !to_fp) {
	...
	while(fgets(line, sizeof(line), from_fp)) {
		fputs(line, to_fp);
	}
