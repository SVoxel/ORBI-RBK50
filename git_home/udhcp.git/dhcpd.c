...
void show_clients_hostname(void)
{
	...
	for (i = 0; i < server_config.max_leases; i++) {
		...
		sprintf(mac_s, "%02x:%02x:%02x:%02x:%02x:%02x", lease->chaddr[0], lease->chaddr[1], lease->chaddr[2], lease->chaddr[3], lease->chaddr[4], lease->chaddr[5]);
		if (strcmp(mac_s, "00:00:00:00:00:00") != 0)
...
{	
	...
	while(1) { /* loop until universe collapses */
		...
		if (raw_server_socket < 0) {
			 ...
			 */
			if ((raw_server_socket = raw_socket(server_config.ifindex, 67)) < 0) {
