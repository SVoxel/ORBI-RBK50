...
#define U_BOOT_VERSION 0x0765fd2
#endif
...
#define DNI_BOARD_DATA2_PARTITION_OFFSET 0x00d80000	/* must same as dni_nand_partition in ofpart.c */
		       ...
		       int nbparts)
{
	...
	mtd_read(master, DNI_BOARD_DATA1_PARTITION_OFFSET + i * master->erasesize +DNI_MODEL_ID_OFFSET, sizeof(dni_model_id) - 1, &ret_len, (void *)dni_model_id);
	mtd_read(master, U_BOOT_VERSION, sizeof(dni_u_boot) - 1,&ret_len, (void *)dni_u_boot);
...
#ifdef SUPPORT_DNI_DUAL_FIRMWARE
	if (strcmp(dni_model_id, "RBR50") == 0 && strcmp(dni_u_boot,"0.4") > 0 ){
