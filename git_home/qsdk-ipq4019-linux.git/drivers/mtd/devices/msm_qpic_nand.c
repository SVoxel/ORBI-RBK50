			     ...
			     struct mtd_oob_ops *ops)
{
	...
	int err, ecc_status = 0, pageerr = 0, rawerr = 0, submitted_num_desc = 0;
	...
	while (rw_params.page_count-- > 0) {
		...
		*/
		if ((pageerr == -EBADMSG) || (pageerr == -EUCLEAN)) {
...
validate_mtd_params_failed:
	...
	if (err){
		pr_err("0x%llx datalen 0x%x ooblen %x err %d corrected %d\n",
		       ...
		       total_ecc_errors);
		if(msm_nand_block_markbad(mtd, from & (~mtd->erasesize + 1)))
			pr_err("fail to mark this block as bad block\n");
	}
