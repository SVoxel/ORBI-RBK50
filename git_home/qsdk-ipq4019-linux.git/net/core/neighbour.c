...
#define NEIGH_GC_MAX_REAP_PER_RUN      100
...
static int neightbl_set(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	tbl->gc_reap_limit = NEIGH_GC_MAX_REAP_PER_RUN;

	printk("Neigh-GC reap limit initialized to %d\n", tbl->gc_reap_limit);
...
static struct neigh_sysctl_table {
	...
	.neigh_vars = {
		...
		[NEIGH_VAR_GC_REAP_LIMIT] = {
			.procname       = "gc_reap_limit",
			.maxlen         = sizeof(int),
			.mode           = 0644,
			.extra1         = &zero,
			.extra2         = &int_max,
			  ...
			  proc_handler *handler)
{
	...
	if (dev) {
		...
		t->neigh_vars[NEIGH_VAR_GC_REAP_LIMIT].data = (int *)(p + 1) + 4;
	}
...
static int __init neigh_init(void)
{
    ...
    printk("Neighbour init complete, with restricted-GC and GC-lock enabled\n");
