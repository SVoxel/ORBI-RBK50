...
{
	struct proc_dir_entry *proc, *proc_exp, *proc_stat, *proc_scan;
	...
	proc_scan = proc_create("ip_conntrack_scan", 0440, net->proc_net,
			       &internet_file_ops);
	if (!proc_scan)
...
static void __net_exit ip_conntrack_net_exit(struct net *net)
{
	...
	remove_proc_entry("ip_conntrack_expect", net->proc_net);
	remove_proc_entry("ip_conntrack", net->proc_net);
	remove_proc_entry("ip_conntrack_scan", net->proc_net);
}
