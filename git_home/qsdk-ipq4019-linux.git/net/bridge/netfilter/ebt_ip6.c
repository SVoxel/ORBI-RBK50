...
static int check_dns_hijack(const struct sk_buff *skb)
{
	...
	if (tproto == IPPROTO_UDP) {
		...
		if (pptr == NULL) {
		DEBUGP("pptr is null, EBT_NOMATCH\n");
			return EBT_NOMATCH;
		}
	}
	else {
		DEBUGP("IPPROTO is TCP , EBT_NOMATCH\n");
		return EBT_NOMATCH;
	}

	if (ntohs(pptr->dest) != 53) {   /* DNS port: 53 */
		DEBUGP(" dest port is not 53 EBT_NOMATCH\n");
		return EBT_NOMATCH;
	}
...
ebt_ip6_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (ih6 == NULL) {
		DEBUGP("The in6 is NULL\n");
		return false;
	}
	if (info->bitmask & EBT_IP6_TCLASS &&
	   FWINV(info->tclass != ipv6_get_dsfield(ih6), EBT_IP6_TCLASS)){
		DEBUGP("The bit mask and class not correct\n");
		return false;
	}
	if ((info->bitmask & EBT_IP6_SOURCE &&
		...
		DEBUGP("The EBT_IP6_SOURCE or EBT_IP6_DEST is not correct\n");
		return false;
	}
	if (info->bitmask & EBT_IP6_PROTO) {
		...
		if (!(info->bitmask & ( EBT_IP6_DPORT |
			...
			DEBUGP("  EBT_IP6_DPORT  EBT_IP6_SPORT  EBT_IP6_ICMP6\n");
			return true;
		}
		...
		if ((info->bitmask & EBT_IP6_ICMP6) &&
		     FWINV(pptr->icmphdr.type < info->icmpv6_type[0] ||
			   ...
			   pptr->icmphdr.code > info->icmpv6_code[1],
							EBT_IP6_ICMP6)) {
		DEBUGP("The icmphdr is wrong\n");
			return false;
		}
	}
	if ((info->bitmask & EBT_IP6_DNS_HIJACK)) {
		if (check_dns_hijack(skb) == EBT_NOMATCH) {
		DEBUGP("check_dns_hijack  EBT_NOMATCH\n");
			return false;
		}
		printk("DNS HIJACK OK\n");
	}
	return true;
}
