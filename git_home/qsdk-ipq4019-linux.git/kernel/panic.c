...
unsigned long reboot_reason_flags = 0;
EXPORT_SYMBOL(reboot_reason_flags);

//EMMC flash necessary
...
int find_position(struct mtd_info  *mtd,long unsigned int offset,int len)
{
	uint8_t buf[1];
	size_t retlen;
	int ret;
...
int log_buf_copy(char *dest, int idx, int size);
...
int console_panic_to_flash(void)
{
	char dni_panic_buf[DNI_PAGE_SIZE]={0};
	...
	dni_ubi_write_for_proc(dni_panic_buf);

	return 0;
}
EXPORT_SYMBOL(console_panic_to_flash);
...
int save_kernel_msg_to_flash(void)
{
	struct mtd_info *mtd = NULL;
	int ret, len = 0, idx = 0;
	int count, retbad;
	...
	else
	{
		...
		while(1)
		{
			...
			if( retbad < 0 ){
				...
				printk(KERN_WARNING "have erased successfully11111111.\n");
		...
		while (1) {
			...
			if(offset >= ((count + 1)*128*1024)){
				...
				if( retbad < 0 ){
					...
					printk(KERN_WARNING "have erased successfully2222222.\n");
...
void panic(const char *fmt, ...)
{
	...
	if(sysctl_save_panic_data_enable == 1)
	{
		...
		if (strcmp(flash_type_name,"EMMC") == 0)
			...
			console_panic_to_flash(); //Save reboot reason to nand flash.
			save_kernel_msg_to_flash(); //Save panic console log to nand flash.
		}
	}
