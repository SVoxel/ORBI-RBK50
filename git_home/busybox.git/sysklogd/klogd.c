...
static void check_send_result(char *info)
{
	int send_success=0;
	FILE *syslog_result_info;
	char info_line[256];
	int need_be_check=0;	

	/* now only 3 type log_info send_to_syslog error sometimes
	.so if the info not include the above 3 type log. It should 
	be return directly*/
	
	if(strstr(info, "site blocked") != NULL || strstr(info, "site allowed") != NULL || strstr(info, "service blocked") || strstr(info, "LAN access from remote"))
		need_be_check=1;
	else
		return;	

	syslog_result_info = fopen("/tmp/log/log_message", "r");
	if (syslog_result_info == NULL)
		return; 									// syslogd error . klogd needn't check the info.

	if (need_be_check == 1)
		sleep(1);  									//waiting syslogd write the info.
		
	while(fgets(info_line,sizeof(info_line),syslog_result_info) && need_be_check==1 )
	{
		if(strstr(info_line, info) != NULL)
		{
			send_success=1;
			break;
		}
	...
	fclose(syslog_result_info);
	if(send_success !=1 && need_be_check == 1 )
	{
		klogd_close();
		closelog();
		klogd_open();
		openlog("kernel", 0, LOG_KERN);
		syslog(4,"%s",info);								// resend_the_log_info	
		system("touch /tmp/log_resend");
	}
}
