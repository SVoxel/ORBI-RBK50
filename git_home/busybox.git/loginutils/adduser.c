...
static void passwd_wrapper(const char *login_name, const char *alg) NORETURN;

static void passwd_wrapper(const char *login_name, const char *alg)
{
	if(alg != NULL)
		BB_EXECLP("passwd", "passwd", alg, login_name, NULL);
	else
...
int adduser_main(int argc UNUSED_PARAM, char **argv)
{
	...
	char *p;
	char *password = NULL; /* For FTP AUTH no Encryption supporting ... */
	unsigned opts;
	char *uid;
	...
	opts = getopt32(argv, "h:g:s:G:DSHu:k:p:a:", &pw.pw_dir, &pw.pw_gecos, &pw.pw_shell, &usegroup, &uid, &skel, &password, &alg);
	if (opts & OPT_UID)
	...
	pw.pw_passwd = password ? password : "x";
	 ...
	 */
	/* fields:     2 3  4 5     6 78 */
	p = xasprintf("%s:%u:0:99999:7:::", pw.pw_passwd, (unsigned)(time(NULL)) / (24*60*60));
	...
	if (!(opts & OPT_DONT_SET_PASS)) {
		...
		if (opts & OPT_ALG) {
			char palg[16]= {0};
			sprintf(palg, "-a %s", alg);
			passwd_wrapper(pw.pw_name, palg);
		}else
			passwd_wrapper(pw.pw_name, NULL);
	}
