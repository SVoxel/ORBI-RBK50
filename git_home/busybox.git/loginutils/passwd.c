...
//usage:     "\n	-s	Single (enable) password"

#include "libbb.h"
#include <syslog.h>
#include <sys/resource.h> /* setrlimit */

static char* new_password(const struct passwd *pw, uid_t myuid, const char *algo, int single)
{
...
int passwd_main(int argc UNUSED_PARAM, char **argv)
{
	enum {
		...
		OPT_single = (1 << 4), /* -s - single password */
	...
	opt = getopt32(argv, "a:luds", &opt_a);
	//argc -= optind;
	argv += optind;
	...
	if (!(opt & OPT_lud)) {
		...
		if(opt & OPT_single)
			newp = new_password(pw, myuid, opt_a, 1);
		else
			newp = new_password(pw, myuid, opt_a, 0);
		if (!newp) {
