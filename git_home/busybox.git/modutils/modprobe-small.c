...
static int insmod(char *options, const char *file);
...
//	argv += optind;
	...
	tmp1 = xstrdup(filename);
	tmp = basename(tmp1);
	lenth = strlen(tmp);

    if (lenth > 3 && tmp[lenth - 3] == '.'
			&& tmp[lenth - 2] == 'k' && tmp[lenth - 1] == 'o')
	{
		lenth -= 3;
		tmp[lenth] = '\0';
	}
//	else if (len > 2 && tmp[len - 2] == '.' && tmp[len - 1] == 'o') {
//		len -= 2;
//		tmp[len] = '\0';
//	}
	else{
		// Please add more suffix processing
	}

	memset(filename, 0, strlen(filename));
    sprintf(filename, "%s.ko", tmp);

	len = MAXINT(ssize_t);
	map = xmalloc_open_zipped_read_close(filename, &len);
	if (!map)
